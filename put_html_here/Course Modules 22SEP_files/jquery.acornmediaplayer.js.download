 ...
 *
 */
function sendEventData(event, url, value) {
    if (url) {
        title = getFileFromURL(url);
    }
    try {
        ga('send', 'event', 'Media', event, title, value);
    } catch (e) {
        ...
        var acornPlayer = function () {
            ...
            var playMedia = function () {
                ...
                if (!acorn.$self.prop('paused')) {
                    sendEventData("play", acorn.$self.find("source").attr("src"), acorn.$self.prop('currentTime'));
            ...
            var startSeek = function (e, ui) {
                ...
                sendEventData("seek", acorn.$self.find("source").attr("src"), acorn.$self.prop('currentTime'));
